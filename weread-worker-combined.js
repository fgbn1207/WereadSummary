  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'POST' && url.pathname === '/mp/articles') {
      try {
        ...
        // 2. 检查 WEREAD_COOKIE 配置
        const cookie = env.WEREAD_COOKIE;
        if (!cookie) {
          return new Response(JSON.stringify({
            error: 'cookie_expired',
            message: '微信读书 Cookie 未配置。请在 Cloudflare Dashboard → Worker → Settings → Variables 中添加 Secret：变量名 WEREAD_COOKIE'
          }), {
            status: 401, headers: { 'Content-Type': 'application/json', ...CORS_HEADERS }
          });
        }

        // 3. 获取公众号基本信息（Agent API）
        let bookInfo = {};
        try {
        ...
        const mpApiUrl = 'https://i.weread.qq.com/book/mpBookArticles?bookId=' + encodeURIComponent(bookId) + '&maxIdx=0&count=20';
        const mpResp = await fetch(mpApiUrl, {
          headers: {
            'Cookie': cookie,
        ...
        if (mpResp.status === 401) {
          return new Response(JSON.stringify({
            error: 'cookie_expired',
            message: '微信读书登录已过期，请重新登录 weread.qq.com 并更新 WEREAD_COOKIE'
          }), {
            status: 401, headers: { 'Content-Type': 'application/json', ...CORS_HEADERS }
          });
        }
        ...
        const rawArticles = mpData.articles || mpData.data || [];
        const articles = rawArticles.map(function(a) {
          var date = '';
          if (a.createTime || a.timestamp) {
            var ts = (a.createTime || a.timestamp) * 1000;
            var d = new Date(ts);
          ...
          return {
            title: a.title || '',
            url: a.url || '',
            deepLink: a.deepLink || a.appUrl || '',
            date: date,
            readCount: a.readCount || a.readStat || 0,
            likeCount: a.likeCount || a.likeNum || 0,
